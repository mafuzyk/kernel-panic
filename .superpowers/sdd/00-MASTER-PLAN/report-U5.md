...
- Probe U5 final headless: exit 0, 59 `PROBE_PASS`, 0 `PROBE_FAIL`, `PROBE_DONE fails=0`.
- Probe U5 final Xvfb: exit 0, 59 `PROBE_PASS`, 0 `PROBE_FAIL`, `PROBE_DONE fails=0`.
...
Não foram comprovados hardware Android/touch físico, leitor de tela nativo, gamepad, escala de texto, alto contraste, reduced flash, PT-BR, export, aprovação visual humana ou performance de dispositivo. A falha de persistência não foi forçada contra production storage; o status contract foi implementado, mas a probe não altera permissões/arquivo do sistema para simular esse erro. Diagnósticos de resources/RID/ObjectDB/text shaping no encerramento continuam resíduos não-gating do baseline.
