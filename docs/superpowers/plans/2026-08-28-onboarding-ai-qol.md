...
- Do not alter wave composition, wave budget, elite chance, base HP, base damage, projectile damage/speed, or spawn limits. Approved difficulty exceptions are the capped `max_alive` ceiling of 10 requested for the denser strategic AI package and the capped cooldown factor in Task 5; both must be covered by harness assertions.
...
- Produces explicit boss distance profiles: ROOT/melee closes distance while SEGFAULT, BLUE SCREEN, and PAGE FAULT/ranged variants maintain a readable retreating distance band. SEGFAULT gains a bounded corruption-shot volley whose projectiles deal the normal direct hit if they reach the player; only a missed shot bursts around mid-path into a pool, without a separate telegraph.
- Reduces the simultaneous alive-enemy ceiling to 10 while preserving the existing lower-wave ramp and all wave composition/budget rolls.
- Produces a bounded OOM_KILLER escape path that can complete at the arena boundary instead of remaining permanently clamped in the corner.

- [ ] **Step 1: Write the failing tests**

Add assertions for cadence values at waves 1, 5, 6, and 30, including the `0.78` floor, and for `Balance.max_alive(1)`, `Balance.max_alive(2)`, and a late wave equaling 10. Add an elite probe that configures one `swift` and one `volatile` enemy with a deterministic test RNG state and asserts the profile hooks expose different movement/tempo behavior and the volatile death hazard remains present. Add a LANCER/SPEWER probe showing the cadence factor is applied to repeated intervals while their telegraph durations remain readable. Add boss probes showing melee ROOT approaches while ranged variants retreat/hold distance, and SEGFAULT's corruption shots travel then either apply direct impact without a pool when intercepted or burst mid-path into a bounded corruption zone when missed. Add an OOM_KILLER flee probe at a clamped edge and assert it releases stolen motes/queues itself for removal instead of remaining stuck.
...
Add the exact capped factor to `Balance` and lower the `max_alive` ceiling to 10 while preserving its current lower-wave ramp. Pass `wave` through `Spawner` as `threat_wave` without changing queue composition. Refactor elite behavior behind small base hooks: `swift` retains its current speed/ghost identity but gets stronger lateral steering and bounded attack reacquisition; `volatile` retains the existing six-orb death burst and gets an explicit arming/pulse visual. Make the boss HOVER steering choose approach for the melee ROOT profile and a retreating distance band for all ranged variants. Add a code-drawn SEGFAULT corruption-shot projectile that travels toward the player; if it is intercepted before its midpoint it uses the existing direct-hit path and creates no zone, otherwise it bursts around mid-path and creates a bounded `CorruptionZone`; do not add a separate pre-telegraph. Fix OOM_KILLER's escape condition to account for the base movement clamp while keeping the existing theft/return behavior. Scale LANCER phase re-entry, SPEWER firing intervals, and repeated ranged-boss cooldowns by the factor; do not shorten attack wind-up telegraphs.
