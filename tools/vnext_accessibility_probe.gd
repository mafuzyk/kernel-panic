...
func _ready() -> void:
	...
	_check((fixture.load(sfx.SAVE_PATH) == OK) and fixture.get_value("progress", "u5_probe_marker", "") == "preserve-me", "profile save preserves unrelated progress")
	...
	var before := int(surface.activation_count)
	...
	_check(int(surface.activation_count) == before + 1, "first reset activation is observable")
