...
func _autotest() -> void:
	...
	await _sec_boot._panel_touch_target_test()
	await _sec_boot._pause_settings_test()
	await _sec_boot._pause_fits_test()
	await _sec_boot._updater_test()
	...
	await _sec_deep._kernel_task_spawn_test()
	...
	await _sec_deep._enemy_ai_test(arena2)
