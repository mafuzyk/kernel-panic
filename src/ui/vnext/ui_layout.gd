...
static func boot(viewport: Vector2, context: RefCounted) -> Dictionary:
	...
	return {"shell": safe, "title": title, "telemetry": telemetry, "illustration": illustration, "boot": boot, "back": back, "boot_label": ">> BOOT PROCESS  [ENTER]" if context.get("density") == "narrow" else ">> BOOT / RUN PROCESS  [ENTER]"}
