...
static func from_player_fixture(player: Dictionary) -> Dictionary:
	...
	return Descriptor.normalize({
		...
		"nested": {
			...
			"page_cache": player.get("page_cache", {}).duplicate(true) if player.get("page_cache", {}) is Dictionary else {},
...
static func from_player(player: Object) -> Dictionary:
	...
	return from_player_fixture({
		...
		"page_cache": player.get("page_cache") if player.get("page_cache") is Dictionary else {},
...
static func from_enemy_fixture(enemy: Dictionary) -> Dictionary:
	...
	var nested := {
		...
		"pair_id": str(enemy.get("pair_id", "")),
		"pair_linked": bool(enemy.get("pair_linked", false)),
		"pair_distance": maxf(float(enemy.get("pair_distance", 0.0)), 0.0),
